...
// El contenido completo ya está reflejado en el canvas y lo empaquetaremos como archivo .dart
...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  List<ScatterData> _buildScatterData() {
    ...
    for (var i = 0; i < principios.length; i++) {
      ...
      list.add(
        ScatterData(x: promEj.clamp(0.0, 5.0), y: yIndex.toDouble(), color: Colors.red, radius: 8),
      );
      list.add(
        ScatterData(x: promGe.clamp(0.0, 5.0), y: yIndex.toDouble(), color: Colors.green, radius: 8),
      );
      list.add(
        ScatterData(x: promMi.clamp(0.0, 5.0), y: yIndex.toDouble(), color: Colors.blue, radius: 8),
      );
    }
...
import 'package:applensys/services/local/dashboard_notifier.dart';
